	...
	case 0:	/* hi/low */
		...
		if ($breach_up || $breach_down) {
			...
			if ($notify) {
				...
				if ($logset == 1) {
					logger($item['name'], $breach_up, ($breach_up ? $item['thold_hi'] : $item['thold_low']), $currentval, $trigger, $item['thold_fail_count'], $url);
				}
			...
			if ($notify) {
				...
				if ($logset == 1) {
					logger($item['name'], 'warning', ($warning_breach_up ? $item['thold_warning_hi'] : $item['thold_warning_low']), $currentval, $warning_trigger, $item['thold_warning_fail_count'], $url);
				}
	...
	case 2:	/* time based */
		...
		if ($breach_up || $breach_down) {
			...
			if ($notify) {
				...
				if ($logset == 1) {
					logger($item['name'], $breach_up, ($breach_up ? $item['time_hi'] : $item['time_low']), $currentval, $trigger, $failures, $url);
				}
			...
			if ($notify) {
				if ($logset == 1) {
					logger($item['name'], 'warning', ($warning_breach_up ? $item['time_warning_hi'] : $item['time_warning_low']), $currentval, $warning_trigger, $warning_failures, $url);
				}
